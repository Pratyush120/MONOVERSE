...
export default function EditorStaffLayout({ children }: { children: ReactNode }) {
  const pathname = usePathname();

  const navItems = [
    { name: "Incoming Queue", href: "/write/editor" },
    { name: "My Assignments", href: "/write/editor/assignments" },
    { name: "Scheduled", href: "/write/editor/scheduled" },
    { name: "Archive", href: "/write/editor/archive" },
  ];
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col md:flex-row pt-[80px]">
      ...
      <aside className="w-full md:w-[280px] border-r border-glass-border-light bg-black/40 flex-shrink-0 flex flex-col h-[calc(100vh-80px)] md:sticky md:top-[80px]">
        <div className="p-[32px]">
          <h2 className="font-display text-[24px] mb-[4px] text-bronze-accent">Editor Desk</h2>
          <p className="font-mono text-[11px] uppercase tracking-[0.1em] text-text-secondary">Internal Staff Only</p>
        </div>
