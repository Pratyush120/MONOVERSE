...
export default function HomeClient({ allEssays }: { allEssays: any[] }) {
  ...
  const featuredEssays = allEssays.slice(0, 1);
  const editorsPicks = allEssays.slice(1, 5);
  const communityEssays: any[] = [];
  
  // Fallbacks if data not yet migrated perfectly
  const heroEssay = featuredEssays.length > 0 ? featuredEssays[0] : allEssays[0];
  const picksList = editorsPicks.length > 0 ? editorsPicks : allEssays.slice(1, 5);
        ...
        <section className="gsap-reveal-section py-[48px] md:py-[80px] relative z-10">
          <div className="max-w-[1440px] mx-auto px-[24px] md:px-[64px]">
            ...
            <EssayCard
              ...
              date={heroEssay.date}
              readTime={heroEssay.readingTime}
